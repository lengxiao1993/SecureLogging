...
def sometx():
    ...
    #... RSCFactory is the class of mintettes
    ...
    tx1 = rscoin.Tx([], [rscoin.OutputTx(k1.id(), 100)]) #__init__(self, inTx=[], outTx=[], R=None):
    ...
    for kv, vv in tx1.get_utxo_out_entries() + tx2.get_utxo_out_entries():     # kv = b'tx.id, tx.pos'/ addrid       vv= b'key_id, value'
        factory.db[kv] = vv                                                    #? set tx1 and tx2 as unpsent transaction for this single mintette
...
def test_TxQuery(sometx):
    ...
    print "Number of Authorities: %s" % len(factory.get_authorities(tx1.id()))  #???get_authorities returns the owners of a transaction
    ...
    assert factory.process_TxQuery(data)
    data2 = (tx4, [tx1.serialize()],                             #?? this data2 is used to test failure case for process_TxQuery ?? 
                            [k1.export()[0]], 
                            [k1.sign(tx3.id())])
